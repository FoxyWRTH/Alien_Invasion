...
def run():
    ...
    bullet_list = Group()
    enemies = Group()
    button_controls.create_army(screen, enemies)
    statis = Statis()
    ...
    while True:  # Бесконечный цикл игры. Обработка действий игрока.
        ...
        button_controls.update(bg_color, screen, player_ship, enemies,
                               bullet_list)
        button_controls.update_bullets(screen, enemies, bullet_list)
        button_controls.update_enemies(statis, screen,
                                       player_ship, enemies, bullet_list)
